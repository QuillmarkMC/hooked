...
execute if score $Temp lobbysigns matches 0 run tellraw @s [{"text": ""},{"text": "[!] ","color": "blue","bold": true},{"text":"Use the command \""},{"text":"/trigger value set x","clickEvent":{"action":"suggest_command","value":"/trigger value set "},"color": "gold","underlined": true},{"text":"\" where x is the custom value you want to set."}]
...
execute if score $Temp lobbysigns matches 0 run tag @s add editing.VictoryTime
