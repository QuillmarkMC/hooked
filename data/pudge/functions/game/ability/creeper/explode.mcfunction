...
execute at @s if score @a[tag=tempAttackerTag,limit=1] creeperExplosion matches 1.. run summon creeper ~ ~ ~ {ExplosionRadius:3,Fuse:0,Invulnerable:1}
execute at @s if score @a[tag=tempAttackerTag,limit=1] creeperExplosion matches 2.. run summon creeper ~ ~ ~ {ExplosionRadius:3,Fuse:0,Invulnerable:1}
execute at @s if score @a[tag=tempAttackerTag,limit=1] creeperExplosion matches 3.. run summon creeper ~ ~ ~ {ExplosionRadius:3,Fuse:0,Invulnerable:1}
