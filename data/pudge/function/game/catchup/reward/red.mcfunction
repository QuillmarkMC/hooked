...
#mercy rule reminder
execute if score $MercyRule timers matches 1 as @a[team=red] run tellraw @s [{"text": ""},{"text": "[!] ","color": "#9E01FE","bold": true},{"translate":"text.game.catchup.mercy.1"},{"translate":"text.game.catchup.mercy.2","clickEvent":{"action":"suggest_command","value":"/trigger surrender"},"hoverEvent":{"action":"show_text","contents":[{"translate":"text.game.catchup.mercy.3","italic":true,"color":"yellow"}]},"color": "gold","underlined": true},{"translate":"text.game.catchup.mercy.4"}]
